import { ImageAttributes } from "./arrayInterface"

const RastlineArr: Array<ImageAttributes> = [
    {
        src: '/assets/rastline/Avrikelj.jpg',
        title: 'Avrikelj',
        sciName: 'Primula auricula',
        description: 'Avrikelj. Medij akvarel in ink',
    },
    {
        src: '/assets/rastline/kranjska_lilija_odzadje.jpg',
        title: 'Kranjska lilija',
        sciName: 'Lilium carniolicum',
        description: 'Kranjska lilija. Medij akvarel in ink, digitalno odzadje',
    },
    {
        src: '/assets/rastline/ilirska_perunika_odzadje.jpg',
        title: 'Ilirska perunika',
        sciName: 'Iris illyrica',
        description: 'Ilirska perunika. Medij akvarel in ink, digitalno odzadje',
    },
    {
        src: '/assets/rastline/Gentiana_verna.jpg',
        title: 'Spomladanski svišč',
        sciName: 'Gentiana verna',
        description: 'Spomladanski svišč. Medij akvarel in ink, digitalno odzadje',
    },
    {
        src: '/assets/rastline/navadna_ciklama.jpeg',
        title: 'Navadna ciklama',
        sciName: 'Cyclamen purpurascens',
        description: 'Navadna ciklama. Medij akvarel in ink, digitalno odzadje',
    },
    {
        src: '/assets/rastline/Ranunculus_acris.jpeg',
        title: 'Ripeča zlatica',
        sciName: 'Ranunculus acris',
        description: 'Ripeča zlatica. Medij akvarel in ink',
    },
    {
        src: '/assets/rastline/1_Iris illyrica.jpg',
        title: 'Ilirska perunika',
        sciName: 'Iris illyrica',
        description: 'Listi ilirske perunike. Medij akvarel.',
    },
    {
        src: '/assets/rastline/2 Iris illyrica.jpg',
        title: 'Ilirska perunika',
        sciName: 'Iris illyrica',
        description: 'Neodprti cvetovi ilirske perunike. Medij akvarel.'
    },
    {
        src: '/assets/rastline/3 Iris illyrica.jpg',
        title: 'Ilirska perunika',
        sciName: 'Iris illyrica',
        description: 'Odprti cvet ilirske perunike. Medij akvarel.'
    },
    {
        src: '/assets/rastline/4 Iris illyrica.jpg',
        title: 'Ilirska perunika',
        sciName: 'Iris illyrica',
        description: 'Plodovi ilirske perunike. Medij akvarel.'
    },
    {
        src: '/assets/rastline/Ailanthus_altissima_sken.jpg',
        title: 'Veliki pajesen',
        sciName: 'Ailanthus altissima',
        description: 'Mladi listi invazivnega velikega pajesna. Medij akvarel.'
    },
    {
        src: '/assets/rastline/Arundo_donax_sken.jpg',
        title: 'Navadna kanela',
        sciName: 'Arundo donax',
        description: 'Navadna kanela ob koncu sezone. Medij Akvarel.'
    },
    {
        src: '/assets/rastline/Aster_squamatus_sken.jpg',
        title: 'Luskasta nebina',
        sciName: 'Aster squamatus',
        description: 'Invazivna vrsta luskasta nebina. Medij akvarel'
    },
    {
        src: '/assets/rastline/Atriplex_portulacoides_sken.jpg',
        title: 'Tolščkasta loboda',
        sciName: 'Atriplex portulacoides',
        description: 'Tolščkasta loboda. Medij akvarel.'
    },
    {
        src: '/assets/rastline/Centaurium_spicatum_sken.jpg',
        title: 'Klasnata tavžentroža',
        sciName: 'Centaurium spicatum',
        description: 'Redka klasnata tavžentroža'
    },
    {
        src: '/assets/rastline/Fraxinus_angustifolia_sken.jpg',
        title: 'Ozkolistni jesen',
        sciName: 'Fraxinus angustifolia',
        description: 'Ozkolisti jesen. Medij akvarel.'
    },
    {
        src: '/assets/rastline/Ginkgo_biloba_akvarel.png',
        title: 'Dvokrpi ginko',
        sciName: 'Ginkgo biloba',
        description: 'Dvokrpi ginko. Medij ink in akvarel'
    },
    {
        src: '/assets/rastline/Ginkgo_biloba_penink.png',
        title: 'Dvokrpi ginko',
        sciName: 'Ginkgo biloba',
        description: 'Dvokrpi ginko. Medij ink.'
    },
    {
        src: '/assets/rastline/Inula_crithmoides_sken.jpg',
        title: 'Obmorski oman',
        sciName: 'Inula crithmoides',
        description: 'Obmorski oman. Medij akvarel.'
    },
    {
        src: '/assets/rastline/Iris illyrica_penink.jpg',
        title: 'Ilirska perunika',
        sciName: 'Iris illyrica',
        description: 'Odprti cvetovi ilirske perunike. Medij ink.'
    },
    {
        src: '/assets/rastline/Juncus_maritimus_sken.jpg',
        title: 'Obmorsko ločje',
        sciName: 'Juncus maritimus',
        description: 'Obmorsko ločje. Medij akvarel.'
    },
    {
        src: '/assets/rastline/Limonium_angustifolium_sken.png',
        title: 'Ozkolistna mrežica',
        sciName: 'Limonium angustifolium',
        description: 'Ozkolistna mrežica. Medij akvarel.'
    },
    {
        src: '/assets/rastline/Linum_maritimum_sken.jpg',
        title: 'Obmorski lan',
        sciName: 'Linum maritimum',
        description: 'Obmorski lan. Medij akvarel'
    },
    {
        src: '/assets/rastline/Olea_europaea.png',
        title: 'Oljka',
        sciName: 'Olea europaea',
        description: 'Oljka. Medij suhi pasteli.'
    },
    {
        src: '/assets/rastline/Phragmites_australis_sken.jpg',
        title: 'Navadni trst',
        sciName: 'Phragmites australis',
        description: 'Navadni trst. Medij akvarel.'
    },
]


export default RastlineArr
